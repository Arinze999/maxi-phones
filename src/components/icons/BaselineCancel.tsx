import React, { SVGProps } from 'react';

/**
 * Props for BaselineCancel icon.
 * @param color - Icon fill color (defaults to 'currentColor')
 * @param width - Icon width & height (defaults to 16px)
 */
export interface BaselineCancelProps extends Omit<SVGProps<SVGSVGElement>, 'width' | 'height'> {
  color?: string;
  width?: number | string;
}

export function BaselineCancel({ color = 'currentColor', width = 16, ...props }: BaselineCancelProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
      width={width}
      height={width}
      {...props}
    >
      <path
        fill={color}
        d="M12 2C6.47 2 2 6.47 2 12s4.47 10 10 10s10-4.47 10-10S17.53 2 12 2m5 13.59L15.59 17L12 13.41L8.41 17L7 15.59L10.59 12L7 8.41L8.41 7L12 10.59L15.59 7L17 8.41L13.41 12z"
      />
    </svg>
  );
}
